...
class GoogleNews:
    ...
    def getpage(self, query, page=1):
        ...
        try:
            if self.__start != "" and self.__end != "":
                ...
                self.url = "https://news.google.com/search?q={query}".format(query=query)
                print(self.url)
            elif self.__period != "":
    ...
    def get_news(self, query, deamplify=False):
        self.url = 'https://news.google.com/search?q={query}&?hl={lang}'.format(lang=self.__lang, query=query)
        try:
            self.req = urllib.request.Request(self.url, headers=self.headers)
            self.response = urllib.request.urlopen(self.req)
            self.page = self.response.read()
